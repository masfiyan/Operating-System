def sjf_non_preemptive(processes, burst_times, arrival_times):
    ...
    # Bubble Sort to sort processes based on arrival time and burst time
    for i in range(n):
        for j in range(0, n-i-1):
            if arrival_times[j] > arrival_times[j+1] or (arrival_times[j] == arrival_times[j+1] and burst_times[j] > burst_times[j+1]):
