def sjf_non_preemptive(processes, burst_times, arrival_times):
    n = len(processes)

    # Selection Sort to sort processes based on arrival time and burst time
    for i in range(n):
        min_index = i
        for j in range(i + 1, n):
            if arrival_times[j] < arrival_times[min_index] or (arrival_times[j] == arrival_times[min_index] and burst_times[j] < burst_times[min_index]):
                min_index = j

        arrival_times[i], arrival_times[min_index] = arrival_times[min_index], arrival_times[i]
        burst_times[i], burst_times[min_index] = burst_times[min_index], burst_times[i]
...
sjf_non_preemptive(processes, burst_times, arrival_times)
